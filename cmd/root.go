...
func initConfig() {
	...
	if err := Config.ReadInConfig(); err == nil {
		...
		case "Micro":
			Config.Set(`images`, map[string]string{
				`ubuntu-latest`: `node:lts-buster`,
				`ubuntu-20.04`:  `node:lts-buster`,
				`ubuntu-18.04`:  `node:lts-buster`,
				`ubuntu-16.04`:  `node:lts-stretch`,
			})
		}
